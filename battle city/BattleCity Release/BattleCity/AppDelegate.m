...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [[GAI sharedInstance] trackerWithTrackingId:GG_TRACK_ID];
...
//	[window_ addSubview:navController_.view];  // 设定 Director 对象不当前窗口的关系,便亍 Director 操作主窗口。
    ...
    //addmob banner
//    _vAd = [[GADBannerView alloc]initWithFrame:CGRectMake(0, 0, window_.rootViewController.view.frame.size.width, 90)];
//    _vAd.adUnitID = ADMOB_KEY;
//    _vAd.rootViewController =  window_.rootViewController;
//    GADRequest *request = [GADRequest request];
//    [_vAd loadRequest:request];
//    
//    [window_.rootViewController.view addSubview:_vAd];
     [self createAndLoadInterstitial];
    
    
    STAStartAppSDK* sdk = [STAStartAppSDK sharedInstance];
    sdk.devID = STARTADD_ACCOUNT_ID;
    sdk.appID = STARTADD_APP_ID;
...
-(void)applicationWillEnterForeground:(UIApplication *)application
{
    ...
    if (![self checkRate] && [self checkTimeRemind]) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"" message:@"Please rate app ^^!" delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes",@"Next time", nil];
        [alert show];
    }
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    switch (buttonIndex) {
        ...
        {
            [self rateApp];
            [[NSUserDefaults standardUserDefaults]setBool:true forKey:@"checkRate"];
        ...
        case 2:
        {
            [[NSUserDefaults standardUserDefaults]setDouble:[[NSDate date] timeIntervalSince1970] forKey:@"timeRmind"];
            [[NSUserDefaults standardUserDefaults]synchronize];
        }
            break;
        default:
...
- (void)rateApp{
    NSString *  iOS7AppStoreURLFormat = [NSString stringWithFormat:@"http://itunes.apple.com/app/id%d",APP_APPLE_ID];
    ;
    NSString *  iOSAppStoreURLFormat =[NSString stringWithFormat:@"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?type=Purple+Software&id=%d",APP_APPLE_ID];
    
    NSString *urlApp = ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0f)? iOS7AppStoreURLFormat: iOSAppStoreURLFormat ;
    
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:urlApp]];
}

- (BOOL)checkTimeRemind {
    double timeStore = [[NSUserDefaults standardUserDefaults]doubleForKey:@"timeRmind"];
    if (timeStore == 0) {
        [[NSUserDefaults standardUserDefaults]setDouble:[[NSDate date] timeIntervalSince1970] forKey:@"timeRmind"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        return false;
    ...
    if (timeNow - timeStore > 24*60*60) {
        return true;
    }
    return false;
}
- (BOOL)checkRate {
    bool check = [[NSUserDefaults standardUserDefaults]boolForKey:@"checkRate"];
    return check;
}
...
- (void)showAdd {
    ...
    if (self.interstitial.isReady) {
        [self.interstitial presentFromRootViewController:window_.rootViewController];
    }else{
        [startAppAd showAd];
        [startAppAd loadAd];
    }
